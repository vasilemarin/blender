    ...
    std::vector<GHOST_PointerInfoWin32> &outPointerInfo, WPARAM wParam, LPARAM lParam)
{
  ...
  if (!outPointerInfo.empty()) {
    m_tabletData = outPointerInfo.back().tabletData;
  }

  return GHOST_kSuccess;
}

GHOST_WintabWin32 *GHOST_WindowWin32::getWintab() const
{
  return m_wintab;
}

void GHOST_WindowWin32::loadWintab()
{
  if (!m_wintab) {
    if (m_wintab = GHOST_WintabWin32::loadWintab(m_hWnd)) {
      /* Enable Wintab if cursor is inside this window. This ensures Wintab is enabled when the
...
{
  return m_tabletInRange ? m_tabletData : GHOST_TABLET_DATA_NONE;
}
