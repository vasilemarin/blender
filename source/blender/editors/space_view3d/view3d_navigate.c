                                         ...
                                         ViewOpsData *r_vod)
{
...

  {
    ...
    r_vod->init.zfac = ED_view3d_calc_zfac(rv3d, tvec, NULL);
  }
...
void viewops_data_end_navigation(bContext *C, ViewOpsData *vod)
{
  ARegion *region;
  if (vod) {
    region = vod->region;
                                    ...
                                    struct wmKeyMapItem *kmi)
{
  ...
  bool use_cursor_init = RNA_boolean_get(ptr, "use_cursor_init");
  if (use_cursor_init) {
  ...
  switch (nav_type) {
    case V3D_ZOOM:
      ...
      delta = RNA_property_is_set(ptr, prop) ? RNA_property_int_get(ptr, prop) : 0;
      return viewzoom_invoke_impl(C, vod, delta, event, event->xy, use_cursor_init);
    case V3D_ROTATE:
