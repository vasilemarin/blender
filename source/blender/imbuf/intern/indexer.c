                                                      ...
                                                      int quality)
{
  ...
  // context->num_transcode_threads = BLI_system_thread_count() / 4;
    ...
    int height)
{
  ...
  avcodec_open2(output_ctx->codec_context, proxy_codec, &opts);
                                                  ...
                                                  float *progress)
{
  ...
  BLI_condition_init(&context->encoder_suspend_cond);
  ...
  BLI_mutex_init(&context->encoder_suspend_mutex);
...
static void index_ffmpeg_encode_resume(FFmpegIndexBuilderContext *context)
{
  ThreadCondition *suspend_cond = &context->encoder_suspend_cond;
  BLI_condition_notify_one(suspend_cond);
}
...
static output_packet_wrap *get_decoded_output_packet_wrap(FFmpegIndexBuilderContext *context,
                                                          uint64_t index)
{
  ThreadMutex *suspend_mutex = &context->encoder_suspend_mutex;
  ThreadCondition *suspend_cond = &context->encoder_suspend_cond;
  BLI_mutex_lock(suspend_mutex);
  ...
  /* Wait until packet is decoded. */
  while (!packet_wrap->is_decoded &&
...
static output_packet_wrap *get_encoded_output_packet_wrap(FFmpegIndexBuilderContext *context,
                                                          uint64_t index)
{
  ...
  /* Wait until packet is encoded. */
  while (!packet_wrap->is_encoded && !context->all_frames_encoded && !*context->stop) {
    BLI_condition_wait(suspend_cond, suspend_mutex);
  }
                                     ...
                                     output_packet_wrap *output_packet)
{
  ...
  for (int size = 0; size < context->num_proxy_sizes; size++) {
    ...
    if (sws_ctx == NULL) {
      output_packet->scaled_frame[size] = av_frame_clone(decoded_frame);
      continue;
    }

    output_packet->scaled_frame[size] = av_frame_alloc();
    avpicture_fill((AVPicture *)output_packet->scaled_frame[size],
    ...
    if (sws_ctx && decoded_frame &&
      ...
      sws_scale(sws_ctx,
                ...
                output_packet->scaled_frame[size]->data,
                output_packet->scaled_frame[size]->linesize);
    }
  }
}
...
static void *index_ffmpeg_decode_scale_packets(void *job_data)
{
  ...
  while (source_packet = index_ffmpeg_transcode_get_read_packet(transcode_job, frame_index)) {
    ...
    if (index_ffmpeg_decode_packet(transcode_ctx, source_packet->input_packet, decoded_frame)) {
      index_ffmpeg_scale_frame(transcode_job, decoded_frame, output_packet);
      ...
      output_packet->pts_from_frame = av_get_pts_from_frame(context->input_ctx->format_context,
                                                            decoded_frame);
      output_packet->is_decoded = true;
      output_packet_frame_index++;
      index_ffmpeg_encode_resume(context);
    }
    frame_index++;
  }
...
static void *index_ffmpeg_encode_frames(void *job_data)
{
  TranscodeJob *transcode_job = job_data;
  FFmpegIndexBuilderContext *context = transcode_job->context;
  int thread_number = transcode_job->thread_number;
  ...
  int decoded_frame_index = thread_number * gop_size;
  int encoded_frame_index = thread_number * gop_size;
  /*  int decoded_frame_index = 0;
    int encoded_frame_index = 0;*/
  output_packet_wrap *output_packet;
  while (output_packet = get_decoded_output_packet_wrap(context, decoded_frame_index)) {
    output_packet_wrap *encoded_packet = get_output_packet_wrap(context, encoded_frame_index);
    ...
    for (int size = 0; size < IMB_PROXY_MAX_SLOT; size++) {
      ...
      /* Encoded packet will be stored in encoded_packet wrap. */
      encoded_packet->output_packet[size] = av_packet_alloc();
      AVPacket *packet = encoded_packet->output_packet[size];

      /* Encoder is fed with frame from output_packet wrap. This can have different index than
       * encoded_packet wrap. */
      AVFrame *frame = output_packet->scaled_frame[size];

      int got_output;
      int ret = avcodec_encode_video2(output_ctx->codec_context, packet, frame, &got_output);
      /*printf("thread %d: pkt %0001d, output pkt %0001d\n",
             thread_number,
             output_packet->frame_index,
             encoded_packet->frame_index);*/
      if (ret < 0) {
        printf("%s\n", av_err2str(ret));
        fprintf(stderr,
      ...
      if (got_output) {
        ...
        encoded_packet->is_encoded = true;

        encoded_frame_index++;
        if (encoded_frame_index % gop_size == 0) {
          encoded_frame_index += gop_size * (threads_total - 1);
        }
      }

      MEM_freeN(output_packet->scaled_frame[size]->data[0]);
      av_frame_free(&output_packet->scaled_frame[size]);
    }

    decoded_frame_index++;
    if (decoded_frame_index % gop_size == 0) {
      decoded_frame_index += gop_size * (threads_total - 1);
    }
  ...
  while (got_output) {
    ...
    for (int size = 0; size < IMB_PROXY_MAX_SLOT; size++) {
      ...
      /*printf("thread %d: pkt %0001d, output pkt %0001d\n",
             thread_number,
             output_packet ? output_packet->frame_index : 666666,
             encoded_packet ? encoded_packet->frame_index : 666666);*/
      if (got_output) {
        if (packet->pts != AV_NOPTS_VALUE) {
          packet->pts = av_rescale_q(packet->pts,
                                     output_ctx->codec_context->time_base,
  ...
  // context->all_frames_encoded = true; //causes write to exit prematurely
  index_ffmpeg_resume_all(context);
...
static void *index_ffmpeg_write_frames(void *job_data)
{
  ...
  int frame_index = 0;
  output_packet_wrap *output_packet_wrap;
  while (output_packet_wrap = get_encoded_output_packet_wrap(context, frame_index)) {
    if (*context->stop) {
      break;
    }

    for (int size = 0; size < context->num_proxy_sizes; size++) {
      AVPacket *output_packet = output_packet_wrap->output_packet[size];
      if (output_packet == NULL) {
        continue;
      }

      if (av_interleaved_write_frame(context->proxy_ctx[size]->output_format, output_packet) !=
          0) {
        fprintf(stderr,
      ...
      av_packet_free(&output_packet_wrap->output_packet[size]);
    }

    uint64_t stream_size = avio_size(context->input_ctx->format_context->pb);
    float next_progress = (float)((int)floor(((double)output_packet_wrap->pos) * 100 /
                                                 ((double)stream_size) +
                                             0.5)) /
                          100;
    ...
    context->last_gop_chunk_written = output_packet_wrap->gop_chunk_index;
...
static TranscodeJob **index_rebuild_ffmpeg_init_jobs(FFmpegIndexBuilderContext *context,
                                                     ...
                                                     ListBase *decode_scale_thread,
                                                     ListBase *encoder_thread,
                                                     ListBase *writer_thread)
{
  BLI_threadpool_init(reader_thread, index_ffmpeg_read_packets, 1);
  BLI_threadpool_init(
      decode_scale_thread, index_ffmpeg_decode_scale_packets, context->num_transcode_threads);
  BLI_threadpool_init(encoder_thread, index_ffmpeg_encode_frames, context->num_transcode_threads);
  BLI_threadpool_init(writer_thread, index_ffmpeg_write_frames, 1);
                                           ...
                                           TranscodeJob **transcode_job_array)
{
  ...
  BLI_threadpool_remove(decode_scale_thread, context);
  BLI_threadpool_end(decode_scale_thread);
  BLI_threadpool_remove(encoder_thread, context);
  BLI_threadpool_end(encoder_thread);
                                ...
                                float *progress)
{
  ...
  TranscodeJob **transcode_job_array = index_rebuild_ffmpeg_init_jobs(
      context, &reader_thread, &decode_scale_thread, &encoder_thread, &writer_thread);

  /* Read packets. */
  BLI_threadpool_insert(&reader_thread, context);

  /* Transcode. Job runs in multiple threads working on chunks separated by I-frames. Encoding must
   * be done in separate thread pool with H264 codec. */
  for (int i = 0; i < context->num_transcode_threads; i++) {
    BLI_threadpool_insert(&decode_scale_thread, transcode_job_array[i]);
    BLI_threadpool_insert(&encoder_thread, transcode_job_array[i]);
  }
  ...
  /* Free jobs. */
  index_rebuild_ffmpeg_free_jobs(context,
                                 &reader_thread,
                                 &decode_scale_thread,
                                 &encoder_thread,
                                 &writer_thread,
