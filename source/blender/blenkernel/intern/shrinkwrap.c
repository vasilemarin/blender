...
static void shrinkwrap_calc_normal_projection(ShrinkwrapCalcData *calc)
{
  ...
  if (calc->aux_target) {
    auxMesh = BKE_modifier_get_evaluated_mesh_from_evaluated_object(NULL, calc->aux_target, false);
    if (!auxMesh) {
      return;
    }
                               ...
                               int numVerts)
{
  ...
  if (smd->target != NULL) {
    ...
    calc.target = BKE_modifier_get_evaluated_mesh_from_evaluated_object(NULL, ob_target, false);
