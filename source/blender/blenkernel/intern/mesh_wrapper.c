...
Mesh *BKE_mesh_wrapper_ensure_subdivision(Depsgraph *depsgraph, const Object *ob, Mesh *me)
{
  ThreadMutex *mesh_eval_mutex = (ThreadMutex *)me->runtime.eval_mutex;
  BLI_mutex_lock(mesh_eval_mutex);
  ...
  ModifierEvalContext eval_context = {
      .object = ob,
      .depsgraph = depsgraph,
      .flag = MOD_APPLY_CPU_SUBDIVISION | (use_render ? MOD_APPLY_RENDER : (ModifierApplyFlag)0),
  };

  /* BKE_modifier_modify_mesh is asserting we do not have such layers. */
  CustomData_free_layers(&me->pdata, CD_NORMAL, me->totpoly);

  Mesh *subdiv_mesh = BKE_modifier_modify_mesh((ModifierData *)smd, &eval_context, me);
