...
class SeparateRGBFunction : public blender::fn::MultiFunction {
 public:
  ...
  static blender::fn::MFSignature create_signature()
  {
    ...
    signature.single_input<blender::ColorGeometry>("Color");
  ...
  {
    const blender::VArray<blender::ColorGeometry> &colors =
        params.readonly_single_input<blender::ColorGeometry>(0, "Color");
    ...
    for (int64_t i : mask) {
      blender::ColorGeometry color = colors[i];
      rs[i] = color.r;
      gs[i] = color.g;
      bs[i] = color.b;
    }
...
{
  static blender::fn::CustomMF_SI_SI_SI_SO<float, float, float, blender::ColorGeometry> fn{
      "Combine RGB",
      [](float r, float g, float b) { return blender::ColorGeometry(r, g, b, 1.0f); }};
  builder.set_matching_fn(fn);
}
